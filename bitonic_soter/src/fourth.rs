...
// <T: Ord + Send> としてトレイト境界に Ord と Send を指定する。
// 引数は可変な Ord と Send を持つ何らかの型の値からなるスライス (配列) の参照と、SortOrder 列挙型。
...
// where 節で T と F のトレイト境界を定義する。
// Send と Sync は並列処理時のメモリ安全を保証するためのトレイト境界。
    ...
    F: Sync + Fn(&T, &T) -> Ordering,
{
    if x.len() > 1 {
        ...
        // xをmid_pointを境にした2つの可変の借用に分割し first と second に束縛する。
        let (mut first, mut second) = x.split_at_mut(mid_point);

        // x の分割後の要素数が閾値を超えていたら、rayon による並列処理を行う。
        if mid_point >= PARALLEL_THRESHOLD {
            rayon::join(
                || do_sort(&mut first, true, comparator),
                || do_sort(&mut second, false, comparator),
            );
        } else {
            do_sort(&mut first, true, comparator);
            do_sort(&mut second, false, comparator);
    ...
    F: Sync + Fn(&T, &T) -> Ordering,
{
    if x.len() > 1 {
        ...
        if mid_point >= PARALLEL_THRESHOLD {
            rayon::join(
                || sub_sort(&mut first, forward, comparator),
                || sub_sort(&mut second, forward, comparator),
            );
        } else {
            sub_sort(&mut first, forward, comparator);
            sub_sort(&mut second, forward, comparator);
